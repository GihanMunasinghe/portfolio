...
/* ---------- daily draft agent ---------- */
async function anthropic(messages, tools, maxTokens = 8000) {
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: "claude-sonnet-5", max_tokens: maxTokens, messages, ...(tools ? { tools } : {}) }),
  });
...
async function dailyDraft() {
  ...
  const system = `You draft blog posts for Gihan Munasinghe (${SITE}), a Singapore-based tech lead with 8+ years in banking, telecom and e-commerce; expert in Java, Spring, Kafka, microservices, AWS/GCP/Kubernetes; also a consultant and educator. Write in his first-person voice: practical, professional, warm. NEVER invent personal anecdotes, employers, projects or metrics. Attribute all news/trends/benchmarks to real sources with inline links.`;
  const ask = `Research (web search) what is currently hot in software engineering and the IT industry. Pick ONE timely, substantive topic that fits Gihan's profile and is NOT similar to these existing posts: ${existing || "(none)"}.

Then respond with ONLY a JSON object (no markdown fences) with keys:
- "slug": kebab-case url slug
- "title": post title (max 80 chars)
- "excerpt": 1-2 sentence card summary
- "readTime": like "7 min read"
- "media": one-word badge (AI, Cloud, Architecture, Security, Java, ...)
- "sources": array of source URLs used
- "html": the FULL article body as HTML using only these tags: <p>, <h2>, <ul>, <ol>, <li>, <strong>, <em>, <a href>, <pre><code>, <blockquote>. 6-10 minute read. Inline-link the sources where cited. No <html>/<head>/<body> wrappers, no images.`;
